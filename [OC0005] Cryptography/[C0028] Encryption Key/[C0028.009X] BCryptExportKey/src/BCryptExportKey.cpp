// Compiled with: cl /EHsc .\BCryptExportKey.cpp bcrypt.lib
// Export key with BCryptExportKey
#include <windows.h>
#include <bcrypt.h>
#include <iostream>
#include <vector>
...
int main() {
    ...
    std::cout << "Exported Key Data:\n";
    for (size_t i = 0; i < exportedKey.size(); i++) {
        std::cout << std::hex << static_cast<int>(exportedKey[i]) << " ";
    }
    std::cout << "\n";
