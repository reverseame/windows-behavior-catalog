// Compiled with: cl /EHsc .\BCryptDecrypt.cpp bcrypt.lib
// Decrypt data with BCryptDecrypt
//#include <ntstatus.h>
#include <windows.h>
#include <bcrypt.h>
#include <iostream>
...
int main() {
    ...
    UCHAR cipherText[128] = {0}, decryptedText[128];
    UCHAR iv[16] = {0}; // Initialization vector (all zero for simplicity).
    DWORD cipherTextLen = 0, dataLen = 0, decryptedTextLen = 0;
    ...
    // Decrypt
    status = BCryptDecrypt(hKey, cipherText, cipherTextLen, NULL, iv, sizeof(iv), decryptedText, sizeof(decryptedText), &decryptedTextLen, 0);
    if (status) {
        std::cerr << "BCryptDecrypt failed.\n";
        BCryptDestroyKey(hKey);
        BCryptCloseAlgorithmProvider(hAlg, 0);
        return 1;
    }

    std::cout << "Decryption succeeded. Plaintext length: " << decryptedTextLen << std::endl;
    std::cout << "Plaintext is: " << plainText << " while deciphertext is: " << decryptedText << std::endl;
